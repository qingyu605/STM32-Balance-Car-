...
**************************************************************************/
void MPU6050_setClockSource(uint8_t source) { IICwriteBits(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source); }
...
void DMP_Init(void) {
    ...
    if (!mpu_init()) {
        ...
        if (!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
...
float *Read_DMP(void) {
    ...
    if (sensors & INV_WXYZ_QUAT) {
        ...
        float q1 = quat[1] / q30;
        float q2 = quat[2] / q30;
        float q3 = quat[3] / q30; // 四元数

        Roll = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3;                                     //???????á????
    ...
    angles[1] = Pitch;
    angles[2] = Yaw;
    return angles; // 返回包含 Roll, Pitch, Yaw 的数组
}
/**************************************************************************
...
// MPU6050的外部中断初始化（INT引脚， PA12， 外部中断配置）
void MPU6050_EXTI_Init(void) {
    ...
    //  	EXTI_Init(&EXTI_InitStructure);	 	//根据EXTI_InitStruct中指定的参数初始化外设EXTI寄存器  //Initialize the external EXTI register based on the parameters
    //  specified in EXTI_InitStruct
    //    NVIC_InitStructure.NVIC_IRQChannel = EXTI15_10_IRQn;			//使能MPU6050所在的外部中断通道  //Enable the external interrupt channel where the MPU6050 is
    //    located
